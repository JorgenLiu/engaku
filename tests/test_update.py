...
class TestUpdate(unittest.TestCase):
    ...
    def test_serena_not_included(self):
        self.assertNotIn("serena", _SKILLS)
    ...
    def test_serena_skill_not_created_by_update(self):
        """run update on a fresh repo, verify serena skill is not created."""
        _git_init(self.tmpdir)
        skill_path = os.path.join(
        ...
        code, out, _ = self._capture_run()
        self.assertEqual(code, 0)
        self.assertFalse(os.path.exists(skill_path), "serena/SKILL.md should not be created")
    ...
    def test_update_does_not_merge_serena_server(self):
        """After init, serena is not present in generated mcp.json."""
        import json
        _git_init(self.tmpdir)
        from engaku.cmd_init import run as init_run
        import io
